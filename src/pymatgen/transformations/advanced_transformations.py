...
class MagOrderingTransformation(AbstractTransformation):
    ...
    def _remove_dummy_species(structure):
        ...
        def find_single_real_neighbor(site, structure, max_distance=0.01):
            min_dist = float("inf")
            closest = None

            for idx, other in enumerate(structure):
                if not isinstance(other.specie, DummySpecies):
                    dist = site.distance(other)

                    if dist < min_dist:
                        min_dist = dist
                        closest = (other, dist, idx)

            if closest and closest[1] <= max_distance:
                return closest
            else:
                return None
        ...
        for idx, site in enumerate(structure):
            if isinstance(site.specie, DummySpecies):
                ...
                ##########################################
                neighbors = find_single_real_neighbor(site, structure)
