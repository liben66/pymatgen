        ...
        for key, setting in settings.items():
            if key == "MAGMOM":
                ...
                for i, site in enumerate(structure):
                    if uic_magmom := self.user_incar_settings.get("MAGMOM", {}):
                        vector = uic_magmom[i]
                        if isinstance(vector, list) and len(vector) == 3:
                            mag.extend(vector)
                        else:
                            raise ValueError(f"Invalid MAGMOM vector for site {i}: {vector}")
                ...
                if hubbard_u:
                    if hasattr(structure[0], key.lower()):
                        ...
                        default_value = 0 if key in {"LDAUU", "LDAUJ"} else -1
                        incar[key] = [
                            (setting.get(sym, default_value) if isinstance(setting.get(sym, default_value), float | int) else 0)
